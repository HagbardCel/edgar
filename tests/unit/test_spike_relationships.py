...
def test_locator_backed_concept_endpoint_identity(
    ...
    identity1, stable1 = endpoint_occurrence_identity(
        concept,
        locs[0],
        canonical_doc_uri=_resolver,  # type: ignore[index]
    )
    identity2, stable2 = endpoint_occurrence_identity(
        concept,
        locs[1],
        canonical_doc_uri=_resolver,  # type: ignore[index]
    )
    ...
    assert identity1["resolved_object"] == {
        "kind": "concept",
...
def test_local_resource_endpoint_identity(model_patches: None, linkbase: dict[str, object]) -> None:
    ...
    identity, stable = endpoint_occurrence_identity(
        labels[0],
        None,
        canonical_doc_uri=_resolver,  # type: ignore[index]
    )
    ...
    assert len(identity["resource_occurrence_hash"]) == 64


def test_relationship_occurrence_hash_endpoint_aware(model_patches: None) -> None:
    arc_hash = "a" * 64
    src = {"endpoint_kind": "direct_object", "resolved_object": {"kind": "concept"}}
    dst1 = {"endpoint_kind": "local_resource", "resource_occurrence_hash": "b" * 64}
    dst2 = {"endpoint_kind": "local_resource", "resource_occurrence_hash": "c" * 64}
...
def test_collect_relationships_exact_base_set_multiplicity(
    ...
    for record in result["resource_records"]:
        ...
        assert record["resource"]["resource_content"]["text"] == "Assets"
