...
The [target architecture package](architecture/README.md) is the **adopted** forward plan ([ADR 0012](adr/0012-adopt-bounded-financial-architecture.md)). **Current implementation phase: M0 — in progress.** Phase 2B/2C remain live production behavior until M1A/M2 change them. Consolidating documents does not authorize later-phase implementation ahead of its gate.
...
- **Adopted target:** [M0–M4 migration](architecture/migration-plan.md) via [ADR 0012](adr/0012-adopt-bounded-financial-architecture.md). **M0 is in progress** (adoption, inventory/restore proof, bounded benchmark, review profile). No applicability/selection or canonical financial observation publication is implemented. Current Phase 2A/2B/2C invariants remain in force until M1A/M2 explicitly change them. **Next gate:** M0 acceptance, then M1A.
