...
## Stable metric keys versus contract revisions

Keep a key when a revision formalizes or corrects the specification of the **same intended analytical quantity**. Create a new key for a distinct measurement a researcher could reasonably request alongside the old one for the same issuer, period and reporting basis. The coexistence question is a review aid, not an automatic naming algorithm: M0 records the intended quantity, changed boundaries, historical usage and rationale for each of the initial eight contracts.

Cash including restricted cash and cash excluding restricted cash are distinct quantities; do not hide that difference as routine versions of one key. Adding explicit basis/sign fields or resolving wording ambiguity may retain a key only when the evidenced intended quantity is preserved. An old ambiguous contract remains preserved as written; do not manufacture a precise historical intention to justify reuse. When intention cannot be established, prefer an explicit new key or defer the release contract.

A new hash still requires independent mapping review; neither a same-key revision nor a new key migrates accepted decisions automatically. Do not retain an inappropriate key merely to exploit same-key version coexistence and evade cross-key conflict checks. A rename, merge or retirement must have explicit review rationale; generic aliases/key-migration machinery remains deferred.
...
The authored reviewed packet is durable knowledge, not a parser result. The request explicitly pins the selected packets; exports retain their exact content. Retain originals when correcting an assessment and select the replacement explicitly; conflicting active inputs fail, rather than choosing the latest filename. Draft/AI-only packets cannot qualify a result. Ownership is explicit: Git authors contracts, PostgreSQL governs mapping decisions, and curator-approved standalone packets are durable **request inputs** for qualification/coverage. Their presence on disk, content hash or `reviewed` flag alone grants no authority; the trusted local curator must approve the exact content and the request must select it. They cannot override mapping acceptance or contract meaning. Save/back up the selected originals and include them in publication snapshots.

For M3 and other current-knowledge queries, explicit reviewed packet pins suffice. For strict M4 `semantic_as_of=S`, the exact reviewed content must already be retained in (a) an accepted mapping revision recorded by S, or (b) a previously completed immutable publication that used that reviewed packet and whose application-recorded publication time is by S. Pin the containing revision/publication as the knowledge-time evidence and verify the content digest, review state and timestamp. A candidate/draft snapshot, bare digest, file modification time, reviewer-entered date or Git author date is insufficient. Changed packet content needs its own qualifying record; an old record cannot timestamp new content. Rejection/notice/conflict checks at the requested knowledge snapshot still apply.

If neither permitted record exists, return `knowledge_time_unknown`; do not silently downgrade a strict request to current knowledge. A new export cannot use itself to establish an earlier knowledge cutoff. Application-recorded publication time comes from the actual completion operation, not a caller-supplied historical date; unknown legacy recording provenance stays unknown. These are trusted-local audit records, not cryptographic proof against an owner falsifying the entire store.

This deliberately replaces the undefined “local recording receipt” option. No `registry.assessment_record` table is required in M3 or the initial M4. A dedicated append-only recording mechanism is deferred until actual strict historical queries are materially blocked by assessments absent from both permitted records. Do not create artificial mappings merely to timestamp packets.
