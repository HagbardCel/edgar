...
5. **Bounded official taxonomy evidence for named publication cases** ([ADR 0013](../adr/0013-publication-critical-taxonomy-evidence.md)). When a named M0/release case requires affirmative concept definition/documentation absent from the resolved filing DTS, M1A resolves the authoritative FASB package/release, acquires it through a controlled path, pins publisher URL + package/release identity + artifact hash, and inspects declaration/documentation in isolation with Arelle as an **external evidence packet**. Do not load the official package into the filing DTS; do not infer package identity solely from the Clark QName namespace string; do not use ambient Arelle cache/package/network fallback. No package index or taxonomy warehouse in M1A — those remain M5. `available` with sufficient affirmative evidence may clear `declaration_definition`; `assessed_absent` after a complete search is honest but does **not** make an exact claim publication-eligible.
...
| Official taxonomy continuity / package index | Repeated cross-release inspection or reuse needs beyond named M1A cases; broader continuity justified | Package index, continuity comparison, optional reuse policies; package/release provenance separate from filing DTS | Exact QNames remain distinct; no silent namespace continuity or reference-target inference; named M1A cases already use direct pinned packages without an index ([ADR 0013](../adr/0013-publication-critical-taxonomy-evidence.md)) |
